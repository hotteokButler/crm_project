...
include_once('solapi_config.php');
...
{
    global $sol_config;
    $apiKey = $sol_config["apiKey"];
    $apiSecret = $sol_config["apiSecret"];
...
{
    global $sol_config;
    $url = "{$sol_config['protocol']}://{$sol_config['domain']}";
    if ($sol_config['prefix']) $url .= $sol_config['prefix'];
    $url .= $resource;
